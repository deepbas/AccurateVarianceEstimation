...
rij_like <- function(i,j,y, R, mu, sig2)
{
  ...
  B <- t(y) %*% y
  rij <- R[i,j]
  #foo <- -(n/2)*determinant(R, logarithm = TRUE)$modulus - sum(diag((solve(R,tol = 1e-10) %*%B)))/2
  #foo <- -(n/2)*determinant(R, logarithm = TRUE)$modulus - sum(diag((solve(R, B))))/2
  #foo <- -(n/2)*determinant(R, logarithm = TRUE)$modulus - sum(diag(chol2inv(chol(R))%*%B))/2
  foo <- -(n/2)*determinant(R, logarithm = TRUE)$modulus - sum(diag(qr.solve(R, tol = 1e-19)%*%B))/2
...
CorMCMC <- function(y, tau2, alpha, beta, port = 1, nsim = 1e3, h = rep(.005, dim(y)[2]*(dim(y)[2] - 1)/2))
{
  ...
  print(acc/nsim)
  return(list("out" = out, "loops" = loops, "acc" = acc/nsim) )
}


lower <- function(vec, D){
  g <- matrix(0,4,4)
  diag(g) = rep(1,4)
  g[lower.tri(g, diag=FALSE)] <- vec
  lower_r <- forceSymmetric(g, uplo = "L")
  mat <- sqrt(D) %*% lower_r %*% sqrt(D)
  mat[lower.tri(mat)]
}
