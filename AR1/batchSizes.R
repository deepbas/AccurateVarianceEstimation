...
est_var <- function(chain, phi, Sigma)
{
	...
	exactBM		<- lapply(1:3, function(k) mcse.multi(chain, size = ceiling(batchObj[k,1]), r = k)$cov)
	secondBM 	<- lapply(1:3, function(k) mcse.multi(chain, size = ceiling(batchObj[k,2]), r = k)$cov)
	firstBM 	<- lapply(1:3, function(k) mcse.multi(chain, size = ceiling(batchObj[k,3]), r = k)$cov)
